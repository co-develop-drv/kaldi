...
# @ARGV是Perl默认用来接收参数的数组，这些参数来源于用户在命令行上输入的参数
for ($x = 1; $x <= 2 && @ARGV > 0; $x++) {
    if ($ARGV[0] eq "-j") {
        shift @ARGV;    #wav.xxx.1 2 ... n.scp
        $num_jobs = shift @ARGV;
        $job_id = shift @ARGV;
...
$error = 0;
$inscp = shift @ARGV;
# print $inscp; # data/mfcc/train/wav.scp
...
# exp/make_mfcc/train/wav_train.1.scp exp/make_mfcc/train/wav_train.2.scp exp/make_mfcc/train/wav_train.3.scp exp/make_mfcc/train/wav_train.4.scp exp/make_mfcc/train/wav_train.5.scp exp/make_mfcc/train/wav_train.6.scp exp/make_mfcc/train/wav_train.7.scp exp/make_mfcc/train/wav_train.8.scp
# print @OUTPUTS;
...
if ($utt2spk_file ne "") {  # We have the --utt2spk option...
    ...
    for ($spkidx = 0; $spkidx < $numspks; $spkidx++) { # 说话人数
...
#         print $scpfile;# exp/make_mfcc/train/wav_train.1 2...n.scp
