...
data=$1 # echo $data : data/mfcc/train

# data/mfcc/train spk2utt utt2spk缺一退出
if [ ! -d $data ]; then
...
# 检查utt2spk格式，每行必须是两个字段，awk -F: '{print NF}' /etc/passwd   每行有多少字段 
...
trap 'rm -rf "$tmpdir"' EXIT HUP INT PIPE TERM #收到各种终止信号后，删除临时目录
...
# 检查非法字符和diff 比较句子文件txt差异
num_utts=`cat $tmpdir/utts | wc -l`
if [ -f $data/text ]; then
  utils/validate_text.pl $data/text || exit 1; #它里面自己注释的很清楚了，验证编码的，并不在意是什么编码，只是要保证字符串长度
...
#segments(<utterance-id> <recording-id> <segment-begin> <segment-end>)存在则wav.scp必须存在
